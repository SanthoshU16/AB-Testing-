...
export class PrivacyComponent implements OnInit, AfterViewInit {
  ...
  private scrollToElement(id: string): void {
    setTimeout(() => {
      ...
      if (element) {
        const headerOffset = 140; // 72px (Navbar) + ~50px (Tabs) + buffer
  ...
  onScroll(): void {
    ...
    // We consider a section active if its top is within the top 40% of the viewport
    ...
    for (const id of sections) {
      ...
      if (element) {
        ...
        // If the top of the section is above our threshold, it's a candidate for "active"
